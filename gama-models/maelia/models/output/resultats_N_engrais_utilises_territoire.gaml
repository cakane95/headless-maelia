...
model resultats_N_engrais_utilises_territoire
...
global{
      action initialisationEcritureFichierresultats_N_engrais_utilises_territoire {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_engrais_utilises_territoire';           
            
            create resultats_N_engrais_utilises_territoire number: 1{
                  do initialisation();
...
species resultats_N_engrais_utilises_territoire parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_engrais_utilises_territoire'+ nomDeLaSimulation + '.csv';  
		...
		loop e over: listeEngraisParOrdreSaisie {
			noms_engrais <- noms_engrais + ";" + e.nomEngrais;
		}
		data <- data + "\nnom_engrais" + noms_engrais;
      ...
      string ecritureFinAnnuelle {
            ...
            loop e over: listeEngraisParOrdreSaisie {
            	float quantite_consommee_annuelle <- quantites_engrais_annuelles[e.nomEngrais];
            	data <- data + ";" + quantite_consommee_annuelle;
            	put 0.0 at: e.nomEngrais in: quantites_engrais_annuelles;
			}
            return data;      
      }
}
