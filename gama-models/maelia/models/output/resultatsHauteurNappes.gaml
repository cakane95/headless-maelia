...
global{
	action initialisationEcritureFichiersHauteurNappes{
		do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers hauteurs de nappes...';		
		
		create resultatsHauteurNappes number: 1{
			do initialisation();
...
species resultatsHauteurNappes parent: ecritureResultats{
	/*
	 * @Overwrite
	 */
	 string initialisationJournalier{			
		...
		nomFichierJournalier <- cheminRelatifDuDossierDeSortieDeSimulation +'/hauteurDeNappe'+ nomDeLaSimulation + '.csv';
		string dataJournaliere <- 'annee';
			
        ask listeZonesHydrographiques{
        	dataJournaliere <- dataJournaliere  +
					 ';hauteurDeNappe[mm]' +'_' + name ;
        }
	 ...
	 */		 
	 string ecritureJournaliere{		 	
		...
		loop zh over: listeZonesHydrographiques{
			...
			data <- data + ';' + zoneHydrographiqueSWAT(zh).hauteurDeNappe with_precision 2;
		 }
