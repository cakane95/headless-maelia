...
model resultats_N_exportation_pailles_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_exportation_pailles_Parcelles {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_exportation_pailles_Parcelles';           
            
            create resultats_N_exportation_pailles_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_exportation_pailles_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationJournalier {   
		nomFichierJournalier <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_exportation_pailles_Parcelles'+ nomDeLaSimulation + '.csv';  
		...
		loop p over: listeParcelles {
			if (empty(idParcelles)) {
				...
				idExploitation <- p.ilot_app.codeExploitationAssociee;
			} else {
				idParcelles <- idParcelles + ";" + p.idParcelle;
				surfaceParcelles <- surfaceParcelles + ";" + p.surface;
				idExploitation <- idExploitation + ";" + p.ilot_app.codeExploitationAssociee;
			}
		}
      ...
      string ecritureJournaliere {
            ...
            loop p over: listeParcelles {
            	write "Biomasse exportée (résultats) = " + parcelleAqYieldNC(p).MSA_exportee_parcelle;
            	data <- data + ";" + parcelleAqYieldNC(p).MSA_exportee_parcelle; // Nlosses_cumul pour avoir tous les N (N2, NO3, NH3)
            	parcelleAqYieldNC(p).MSA_exportee_parcelle <- 0.0;
            }
