...
model resultats_N_quantitesProduits_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_quantitesProduits_Parcelles{
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_quantitesProduits_Parcelles';           
            
            create resultats_N_quantitesProduits_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_quantitesProduits_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_quantitesProduits_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	string dataParcelle;
            	loop eng over: Engrais {
            		if (p.quantitesProduits[eng.nomEngrais] > 0) {
            			if (dataParcelle = nil) {
            				dataParcelle <- string(eng.nomEngrais + "!" + p.quantitesProduits[eng.nomEngrais]);
            			} else {
            				dataParcelle <- string(dataParcelle + "|" + eng.nomEngrais + "!" + p.quantitesProduits[eng.nomEngrais]);
            			}
            			p.quantitesProduits[eng.nomEngrais] <- 0;
            		}
            	}
