...
global{
	action initialisationEcritureFichiersECO_itk{
		do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers ECO / itk...';		
		
		create resultatsECO_itk number: 1{
			do initialisation();
...
species resultatsECO_itk parent: ecritureResultats{
	/*
	 * @Overwrite
	 */
	 string initialisationFinAnnuel{	
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/eco_itk'+ nomDeLaSimulation + '.csv';	
		string data <- '' + detailSimulation + '\nannee'; //
		loop it over: (itk as list){ 
			loop scenarioPrix over:listScenarioPrix{
				if(scenarioPrix = ''){
					data <- data + ';MB_' + it.nomPourAffichage + ';MN_' + it.nomPourAffichage;
				}else{
					data <- data + ';MB_' +scenarioPrix + '_' + it.nomPourAffichage + ';MN_' +scenarioPrix + '_' + it.nomPourAffichage;
				}
			}
		}
		return data;
	 ...
	 string ecritureFinAnnuelle{			 		 			 		
		...
		loop it over: (itk as list){
			loop scenarioPrix over:listScenarioPrix{
				...
				float prixDeLEspece <- (tmp at it.especeCultiveeITK);
				margeBrute <- margeBrute + Prod * prixDeLEspece; 
