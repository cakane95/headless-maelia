...
model resultats_N_nSemisCultures_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_nSemisCultures_Parcelles{
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_nSemisCultures_Parcelles';           
            
            create resultats_N_nSemisCultures_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_nSemisCultures_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_nSemisCultures_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	string dataParcelle;
            	loop esp over: especeCultivee {
            		if (p.nSemisCultures[esp.idEspeceCultivee] > 0) {
            			if (dataParcelle = nil) {
            				dataParcelle <- string(esp.idEspeceCultivee + "!" + p.nSemisCultures[esp.idEspeceCultivee]);
            			} else {
            				dataParcelle <- string(dataParcelle + "|" + esp.idEspeceCultivee + "!" + p.nSemisCultures[esp.idEspeceCultivee]);
            			}
            			p.nSemisCultures[esp.idEspeceCultivee] <- 0;
            		}
            	}
            	data <- data + ";" + dataParcelle; 
			}
            return data;      
      }
}
