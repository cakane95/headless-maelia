...
model resultats_N_NH3_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_NH3_Parcelles {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_NH3_Parcelles';           
            
            create resultats_N_NH3_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_NH3_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_NH3_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	data <- data + ";" + parcelleAqYieldNC(p).N_nh3_tot_cumul;
			}
            return data;      
      }
}
