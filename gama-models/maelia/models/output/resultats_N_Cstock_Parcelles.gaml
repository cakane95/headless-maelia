...
model resultats_N_Cstock_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_Cstock_Parcelles {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_Cstock_Parcelles';           
            
            create resultats_N_Cstock_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_Cstock_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_Cstock_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	data <- data + ";" + parcelleAqYieldNC(p).C_stock_cumul;
			}
            return data;      
      }
}
