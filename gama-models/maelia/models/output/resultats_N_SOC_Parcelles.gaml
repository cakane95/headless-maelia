...
model resultats_N_SOC_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_SOC_Parcelles{
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_SOC_Parcelles';           
            
            create resultats_N_SOC_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_SOC_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_SOC_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	data <- data + ";" + mean(parcelleAqYieldNC(p).SOC_Clay_ratio_year); // "The SOC:clay ratio, however, appears to be a relevant criteria when considering soil structure quality" - Johannes 2017 Geoderma "The SOC:clay ratio, however, appears to be a relevant criteria when considering soil structure quality"
				parcelleAqYieldNC(p).SOC_Clay_ratio_year <- [];
			}
            return data;      
      }
}
