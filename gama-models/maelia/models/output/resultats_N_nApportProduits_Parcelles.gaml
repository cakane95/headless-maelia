...
model resultats_N_nApportProduits_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_nApportProduits_Parcelles{
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_nApportProduits_Parcelles';           
            
            create resultats_N_nApportProduits_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_nApportProduits_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_nApportProduits_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	...
            	loop eng over: Engrais {
            		if (p.nApportProduits[eng.nomEngrais] > 0) {
            			if (dataParcelle = nil) {
            				dataParcelle <- string(eng.nomEngrais + "!" + p.nApportProduits[eng.nomEngrais]);
            			} else {
            				dataParcelle <- string(dataParcelle + "|" + eng.nomEngrais + "!" + p.nApportProduits[eng.nomEngrais]);
            			}
            			p.nApportProduits[eng.nomEngrais] <- 0;
            		}
            	}
