...
model resultats_N_QNfix_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_N_QNfix_Parcelles {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_N_QNfix_Parcelles';           
            
            create resultats_N_QNfix_Parcelles number: 1{
                  do initialisation();
...
species resultats_N_QNfix_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_N_QNfix_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	data <- data + ";" + parcelleAqYieldNC(p).QNfix_cumul;
            	parcelleAqYieldNC(p).QNfix_cumul <- 0.0;
			}
            return data;      
      }
}
