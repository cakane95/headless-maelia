...
model resultatsRetenuesVolumeActuelJour
...
species resultatsRetenuesVolumeActuelJour parent: ecritureResultats{
	 ...
	 */
	 string initialisationJournalier{			
		...
		nomFichierJournalier <- cheminRelatifDuDossierDeSortieDeSimulation +'/volumeActuelRetenues'+ nomDeLaSimulation + '.csv';
		...
		ask listeRetenuesCollinairesAsuivre{
			dataJournaliere <- dataJournaliere + ";" +int(self.volumeMax);
		}
		dataJournaliere <- dataJournaliere + '\nSurface';
		ask listeRetenuesCollinairesAsuivre{
			dataJournaliere <- dataJournaliere + ";" +int(self.surface);
		}
		dataJournaliere <- dataJournaliere + '\nTypeRet';
		ask listeRetenuesCollinairesAsuivre{
			dataJournaliere <- dataJournaliere + ";" +self.typeOfRet;
		}
		dataJournaliere <- dataJournaliere + '\nUsageAgricole';
		ask listeRetenuesCollinairesAsuivre{
			bool usageAgricole <- false;
			if mapEquipementsCaptageAssocies[IRR]!= nil {
				if(length(mapEquipementsCaptageAssocies at IRR)> 0){ // Si RET a usage Agricole
					usageAgricole <- true;
				}
			}
			dataJournaliere <- dataJournaliere + ";" + usageAgricole;
		}
