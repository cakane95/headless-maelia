...
model resultats_prixFerti_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_prixFerti_Parcelles {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_prixFerti_Parcelles';           
            
            create resultats_prixFerti_Parcelles number: 1{
                  do initialisation();
...
species resultats_prixFerti_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_prixFerti_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	data <- data + ";" + parcelleAqYieldNC(p).prixFerti_cumul; // €/ha
			}
            return data;      
      }
}
