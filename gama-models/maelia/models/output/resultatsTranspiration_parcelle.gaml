...
***************************************************************************/
/**
*  resultatsTranspiration_parcelle
*  Author: Jean Villerd
*  Description: 
 */

model resultatsTranspiration_parcelle
...
global{
      action initialisationEcritureFichiersTranspiration_parcelle {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers transpiration par parcelle...';           
            
            create resultatsTranspiration_parcelle number: 1{
                  do initialisation();
...
species resultatsTranspiration_parcelle parent: ecritureResultats {
      /*
            * @Overwrite
            */
      string initialisationJournalier{   
            nomFichierJournalier <- cheminRelatifDuDossierDeSortieDeSimulation +'/transpiration_parcelle'+ nomDeLaSimulation + '.csv';  
            
            // Ligne titre
            string data <- '' + detailSimulation + '\njour;exploitation;parcelle;transpiration reelle [mm];transpiration max [mm];drain [mm];evaporation [mm]';
               ...
               // write "exploitation = " + exploit.id;
                ...
                loop parc over: listeParcelles1Exploit {
	                      ...
	                      data <- data + dateCour.annee + '/' + dateCour.mois + '/' + dateCour.jour + ";" +
	                                 exploit.id+";" + 
	                                 parc.idParcelle+";"+
	                                 parcelleAqYield(parc).transpirationR with_precision 2 +";"+
	                                 parcelleAqYield(parc).getTranspirationMax() with_precision 2 +";"+
	                                 parcelleAqYield(parc).drain with_precision 2 +";"+
	                                 parcelleAqYield(parc).evaporation with_precision 2 +";";	                                 
                }
          }
