...
model resultats_tpsWFerti_Parcelles
...
global{
      action initialisationEcritureFichiersresultats_tpsWFerti_Parcelles {
            do ecritureConsolePourDebug isAfficherTemps: true chaineAEcrire: 'Initialisation ecriture fichiers resultats_tpsWFerti_Parcelles';           
            
            create resultats_tpsWFerti_Parcelles number: 1{
                  do initialisation();
...
species resultats_tpsWFerti_Parcelles parent: ecritureResultats {
      /*
            * @Overwrite
            */
            
      string initialisationFinAnnuel {   
		nomFichierFinAnnuel <- cheminRelatifDuDossierDeSortieDeSimulation +'/resultats_tpsWFerti_Parcelles'+ nomDeLaSimulation + '.csv';  
      ...
      string ecritureFinAnnuelle {
            ...
            loop p over: listeParcelles {
            	data <- data + ";" + parcelleAqYieldNC(p).tps_travail_Ferti_cumul; //  h/ha
