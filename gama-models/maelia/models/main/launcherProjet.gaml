...
/**
 * launcherProjet.gaml — Launcher des RUNS DE PROJET de la plateforme (scénarios).
 *
 * Copie exacte de launcherTest.gaml (dont les défauts sont VALIDÉS par le test headless
 * qui fonctionne sur l'échantillon SASSEME) : scénario minimal par défaut. La plateforme
 * n'envoie au `load` QUE les écarts au défaut (paramètres du scénario) plus les paramètres
 * système (GamaParameterBuilder) : cheminRacineMaelia, cheminModeleVersDonnees
 * (includes matérialisés du projet), nomDecoupageZonePourLectureFichiers '' (includes à
 * plat), idSimulationAPI (= runId, pilote le dossier de sortie), nomSimulation.
 *
 * Mêmes choix headless que launcherTest :
 *   - experiment `until: simulationTerminee` (global posé par main.gaml en fin de run)
 *     => GAMA émet SimulationEnded et le worker termine proprement,
 *   - pas de bloc `output { display ... }` (inutile/risqué en headless).
 */
model launcherprojet
...
experiment simulationProjet type: gui until: simulationTerminee {
 ...
 /* ---------------------------------- PARAMETRES GENERAUX ------------------------------------------------------------------*/
	...
	parameter 'nbAnneesSimulation : ' 			var: nbAnneesSimulation 		<- 1; // TEST: run court (1 an)
	...
	// (la plateforme envoie toujours nomSimulation = 8 premiers caractères du runId)
	parameter 'nomSimulation : ' 				var: nomSimulation 				<- "projet";

	// nom territoire : includes posés à plat sous includes/ -> territoire vide (cf. en-tête)
	parameter 'nomDecoupageZonePourLectureFichiers : ' 	var: nomDecoupageZonePourLectureFichiers 	<- ''; // TEST: includes à plat
	...
	// déclaré pour autoriser l'override depuis le catalogue de paramètres (défaut modèle = false)
	parameter 'executionViaAPI'					var: executionViaAPI					<- false;
 ...
 /* ---------------------------------- PARAMETRES MODELE AGRICOLE ------------------------------------------------------------------*/
	...
	parameter 'Niveau scalaire de gestion des stocks d engrais'	var: gestionStocksEngrais 			<- "territoire";
	...
	// paramètres sol
	parameter 'fraction SOM inerte fonction du %MO '	var: option_Finert_calc						<- false;
 ...
 /* ---------------------------------- SORTIES ------------------------------------------------------------------*/
	...
	// Pas de bloc `output { display ... }` : exécution headless pilotée par load/play.
	// La progression est suivie via les messages console (`write`) du modèle et la
	// condition d'arrêt `until: simulationTerminee` ci-dessus.
}
