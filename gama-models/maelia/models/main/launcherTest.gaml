...
/**
 * launcherTest.gaml — Launcher de TEST headless du modèle MAELIA réel.
 *
 * But : valider la chaîne front <-> back <-> RabbitMQ <-> GAMA <-> MAELIA avec le
 * VRAI modèle et les includes de base posés dans gama-workspace/maelia/includes/
 * (données échantillon SASSEME), sur le même principe que test/models/simple_test.gaml.
 *
 * Calqué sur launcherSasseme.gaml. Différences :
 *   - territoire vide : les includes sont posés à plat sous includes/ (pas de sous-dossier includes_xxx),
 *     donc nomDecoupageZonePourLectureFichiers <- '' -> chemins = <racine>/includes//modeleCommun/...
 *   - nbAnneesSimulation <- 1 (run court),
 *   - experiment headless : `until: simulationTerminee` (global posé par main.gaml en fin de run)
 *     => GAMA émet SimulationEnded et le worker termine proprement,
 *   - pas de bloc `output { display ... }` (inutile/risqué en headless).
 * Les chemins (cheminRacineMaelia "../../") sont laissés par défaut : résolus relativement
 * au fichier modèle, ils pointent vers gama-workspace/maelia/.
 */
model launchertest
...
experiment test_maelia type: gui until: simulationTerminee {
 ...
 /* ---------------------------------- PARAMETRES GENERAUX ------------------------------------------------------------------*/
	...
	parameter 'nomSimulation : ' 				var: nomSimulation 				<- "test";
