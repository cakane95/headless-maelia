 ...
 *  launcherTest
 *  Author: Jean Villerd
 *  Description: Duplication de launcherBase.gaml pour l'execution headless.
 *
 *  Strictement identique a launcherBase (memes parametres, mêmes valeurs par defaut),
 *  a trois details pres, tous imposes par le mode headless :
 *    - `model launcherTest` : deux fichiers ne peuvent pas declarer le meme modele ;
 *    - experiment `test_maelia ... until: simulationTerminee` : main.gaml pose ce
 *      booleen en fin de run, GAMA emet alors SimulationEnded et le worker termine ;
 *    - pas de bloc `output { display ... }` : purement GUI.
 *
 *  Les parametres metier ne sont PAS figes ici : l'interface d'administration les
 *  surcharge au `load`. Le worker impose en plus idSimulationAPI=<runId>, ce qui fait
 *  ecrire les sorties dans models/main/log/<runId> (cf. action majChemins).
 */
...
experiment test_maelia type: gui benchmark:false until: simulationTerminee {
